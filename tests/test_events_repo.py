...
async def test_postpone_event(db_conn):
    ...
    event_id = await create_event(db_conn, user_id=user_id, title="Postpone me", event_datetime=dt)
    ok = await postpone_event(db_conn, event_id, hours=1)
    assert ok is True
    event = await get_event_by_id(db_conn, event_id)
    expected = dt + timedelta(hours=1)
    assert event.event_datetime == expected
...
async def test_advance_recurring_weekly(db_conn):
    ...
    dt = datetime(2025, 10, 13, 9, 0)  # Monday
    event_id = await create_event(
        db_conn,
        user_id=user_id,
        title="Weekly",
    ...
    assert (event.event_datetime - dt).days == 7


@pytest.mark.asyncio
async def test_get_events_to_notify(db_conn):
    user_id = await get_or_create_user(db_conn, 555)
    now = datetime(2025, 11, 20, 14, 30)
    event_id = await create_event(db_conn, user_id=user_id, title="Due now", event_datetime=now)
    events = await get_events_to_notify(db_conn, now)
    assert len(events) == 1
    assert events[0][2].id == event_id
    assert events[0][2].title == "Due now"

    await mark_notified(db_conn, event_id, now)
    events2 = await get_events_to_notify(db_conn, now)
